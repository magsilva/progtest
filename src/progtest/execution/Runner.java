...
public class Runner {
	...
	public static void run(Assignment assignment, UploadedFile uf)
		...
		makeDirectories(oracleDir, packageDir, sourceDir, programDir, testsDir, reportsDir, pitiDir, null, null, null);
		...
		execute(assignment.getCriteria(), oracleDir, programDir, testsDir, pitiDir);
	...
	public static void run(Evaluation evaluation, UploadedFile uf)
		...
		File packageDir = new File(Directories.getPackageDirPath(assignment, student));
		File sourceDir = new File(Directories.getSourceDirPath(assignment, student));
		File programDir = new File(Directories.getProgramDirPath(assignment, student));
		...
		File oracleProgramDir = new File(Directories.getProgramDirPath(assignment));
		File oracleTestsDir = new File(Directories.getTestsDirPath(assignment));
		
		makeDirectories(studentDir, packageDir, sourceDir, programDir, testsDir, reportsDir, null, pstsDir, pitsDir, pstiDir);
		...
		execute(assignment.getCriteria(), studentDir, programDir, testsDir, pstsDir);
		
		execute(assignment.getCriteria(), studentDir, oracleProgramDir, testsDir, pitsDir);
		
		execute(assignment.getCriteria(), studentDir, programDir, oracleTestsDir, pstiDir);
	...
	private static void makeDirectories(File rootDir, File packageDir, File sourceDir, File programDir,
			File testsDir, File reportsDir, File pitiDir, File pstsDir, File pitsDir, File pstiDir) {
	...
	private static void execute(List<Criterion> criteria, File oracleDir,
			File sourceDir, File testsDir, File pitiDir) {
