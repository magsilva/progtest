public class TabelaHash {
	...
	public TabelaHash() { // alterado para ficar com parametros estaticos assim
		// como o programa procedimental
		int m = 7;
		int maxTamChave = 11;
		this.M = m;
		this.tabela = new Celula[this.M];
		for (int i = 0; i < this.M; i++)
			this.tabela[i] = null; // @{\it vazio}@
	...
	public void insere(String chave, Object item) {
		if (this.pesquisa(chave) == null) {
			...
			int indice = inicial;
			int i = 0;
			while (this.tabela[indice] != null && !this.tabela[indice].retirado
					&& i < this.M)
				indice = (inicial + (++i)) % this.M;
			if (i < this.M)
